...
@section('description', 'Platform CMS modern dan mudah digunakan untuk mengelola konten website Anda')

@section('content')
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8 {{ isset($sidebarLeftWidgets) && $sidebarLeftWidgets->count() > 0 ? 'lg:ml-0' : '' }} {{ isset($sidebarRightWidgets) && $sidebarRightWidgets->count() > 0 ? 'lg:mr-0' : '' }}">
    <!-- Hero Section -->
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="order-2 lg:order-1">
                    ...
                    <div class="mt-10 flex flex-col sm:flex-row gap-4">
                        <a href="{{ route('panel.dashboard') }}" class="btn-gradient text-white font-semibold py-3 px-8 rounded-lg text-center shadow-lg hover:shadow-xl transition-shadow">
                            Dashboard
                        </a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 transition-all hover:shadow-lg border border-gray-100 dark:border-gray-700 card-hover">
                    <div class="flex items-start mb-4">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $widget->name }}</h3>
                    ...
                    @if($widget->type === 'html')
                        <div class="prose prose-indigo dark:prose-invert max-w-none text-gray-700 dark:text-gray-300">{!! $widget->content !!}</div>
                    @elseif($widget->type === 'text')
                        <div class="text-gray-700 dark:text-gray-300">{{ Str::limit(strip_tags($widget->content ?? ''), 150, '...') }}</div>
                    ...
                    <div class="mt-4 flex justify-between items-center">
                        ...
                        <span class="inline-block text-xs px-2 py-1 rounded-full 
                            {{ $widget->status === 'aktif' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' }}">
                            {{ $widget->status }}
                        </span>
                    </div>
                </div>
    ...
    <section class="py-20">
        <div class="max-w-7xl mx-auto text-center">
            ...
            <a href="{{ route('panel.dashboard') }}" class="btn-gradient text-white font-bold py-3 px-8 rounded-lg inline-block shadow-lg hover:shadow-xl transition-all">
                Akses Dashboard
            </a>
        </div>
    </section>
